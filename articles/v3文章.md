...
Claude Code 的解法是 **Task 工具**——让主代理像项目经理一样，把子任务派发给专门的"子代理"（Subagent），各自独立完成后再汇报结果。v3 版 mini Claude Code 用最精简的方式复现了这个核心机制：**约 150 行新增代码，还原核心原理**。
...
3. **递归复用而非重复实现**
   ...
   Claude Code 的设计也是如此：agent loop 被主代理和子代理共用（但隔离用各自的 Context、tools）
...
| 机制 | Claude Code | mini Claude Code v3 |
|------|-------------------|--------------|
